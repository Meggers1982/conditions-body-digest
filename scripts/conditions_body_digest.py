...
def get_issns(selected_cats: list[str]) -> list[str]:
    ...
    for cat in selected_cats:
        csv_path = get_csv_path(cat)
        if not csv_path.exists():
            print(f"  Warning: CSV not found for category '{cat}': {csv_path}")
            continue
    ...
    try:
        headers = {
            "User-Agent": (
                ...
                "+https://github.com/meggers1982/conditions-body-digest)"
            ),
...
"""
    ...
    prompt = f"""You are a science writer and editor for a Conditions & Body Research Digest, writing for a journalist audience covering women's health conditions, mental health, skin, and reproductive health.

Your readers pitch to publications like Women's Health Magazine, Health, Self, Allure, Glamour, and similar women's lifestyle and wellness outlets.
...
{{
  ...
  "summary": "2-4 sentences: what researchers did, who participated (N=X, demographics if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for women specifically. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, no sex-disaggregated data — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for women's lifestyle journalism",
  "pitch_angles": [
    {{
      "publication_type": "e.g. Women's Health Magazine / Health / Self / Allure / General women's wellness",
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why surprising or useful, lifestyle hook",
...
- Flag explicitly if the study did not break down results by sex/gender
- For cancer studies: be precise about cancer type, stage, and treatment — do not generalize
- For mental health studies: avoid stigmatizing language, prefer person-first framing
- For reproductive health: flag if findings are specific to a narrow population (e.g. IVF patients, PCOS only)
...
relevance_score rubric (1–10): start at 5, then adjust:
  ...
  +2 human subjects, relevant to women (N≥100)
  +1 clear lifestyle or personal relevance hook
  +1 clean design (RCT, longitudinal, large cohort)
  +1 directly addresses a condition women disproportionately experience (autoimmune, anxiety, thyroid, skin conditions, reproductive health)
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Conditions & Body Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'} ({category}{chunk_label})"
    html = f"""<!DOCTYPE html>
<html>
...
def main():
    ...
    print("=" * 60)
    print("Conditions & Body Research Digest")
